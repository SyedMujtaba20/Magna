  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (templateRendererRef.current && templateCameraRef.current) {
        templateRendererRef.current.render(
          templateSceneRef.current,
          templateCameraRef.current
        );
      }
      if (comparisonRendererRef.current && comparisonCameraRef.current) {
        comparisonRendererRef.current.render(
          comparisonSceneRef.current,
          comparisonCameraRef.current
        );
      }
    };
    animate();
    ...
    return () => {
      ...
      if (templateCanvasRef.current && templateControls) {
        ...
        canvas.removeEventListener(
          "mousedown",
          templateControls.handleMouseDown
        );
        canvas.removeEventListener(
          "mousemove",
          templateControls.handleMouseMove
        );
      ...
      if (comparisonCanvasRef.current && comparisonControls) {
        ...
        canvas.removeEventListener(
          "mousedown",
          comparisonControls.handleMouseDown
        );
        canvas.removeEventListener(
          "mousemove",
          comparisonControls.handleMouseMove
        );
    ...
    points.forEach((point) => {
      ...
      if (isTemplate) {
        ...
        const normalizedThickness =
          (point.thickness - minThickness) / (maxThickness - minThickness);
        let color;
  ...
  useEffect(() => {
    ...
    if (templateSceneRef.current) {
      ...
      if (referenceFile && fileDataCache.has(referenceFile.name)) {
        ...
        const templateMesh = createPointCloud(
          points,
          minThickness,
          maxThickness,
          true
    ...
    if (comparisonSceneRef.current) {
      ...
      if (compareFile && fileDataCache.has(compareFile.name)) {
        ...
        const comparisonMesh = createPointCloud(
          points,
          minThickness,
          maxThickness,
          false
      ...
      >
        ...
        <div style={{ display: "flex", height: "calc(100vh - 60px)" }}>
          ...
          >
            ...
            >
              {t("comparison.templateLabel")}: {referenceFile?.name}
          ...
          >
            ...
            >
              {t("comparison.comparisonLabel")}: {compareFile?.name}
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  ⚠️ {t("comparison.criticalWarningTitle")}
                </div>
                ...
                >
                  {t("comparison.criticalWarningDesc")}
                </div>
              </div>
              ...
              {compareFile && fileDataCache.has(compareFile.name) && (
                ...
                >
                  ...
                  >
                    {t("comparison.measurementStats")}
                    {/* Measurement Statistics: */}
                  </div>
                  <div style={{ marginBottom: "8px", color: "#D1D5DB" }}>
                    <strong style={{ color: "#F9FAFB" }}>Min Thickness:</strong>{" "}
                    {fileDataCache
                      .get(compareFile.name)
                      .minThickness?.toFixed(2) || "N/A"}{" "}
                    mm
                  </div>
                  <div style={{ marginBottom: "8px", color: "#D1D5DB" }}>
                    <strong style={{ color: "#F9FAFB" }}>Max Thickness:</strong>{" "}
                    {fileDataCache
                      .get(compareFile.name)
                      .maxThickness?.toFixed(2) || "N/A"}{" "}
                    mm
                  </div>
                  <div style={{ marginBottom: "8px", color: "#D1D5DB" }}>
                    <strong style={{ color: "#F9FAFB" }}>Data Points:</strong>{" "}
                    {fileDataCache
                      .get(compareFile.name)
                      .points?.length?.toLocaleString() || 0}
                  </div>
                  <div style={{ color: "#D1D5DB" }}>
                    ...
                    {(
                      (fileDataCache.get(compareFile.name).maxThickness || 0) -
                      (fileDataCache.get(compareFile.name).minThickness || 0)
                    ).toFixed(2)}{" "}
                    mm
                  </div>
                </div>
        ...
        >
          ...
          >
            <div>
              ...
              >
                {t("comparison.template")}
              </label>
              <select
                value={referenceFile?.name || ""}
                onChange={(e) =>
                  setReferenceFile(
                    files.find((f) => f.name === e.target.value) || null
                  )
                }
            ...
            <button
              ...
              style={{
                backgroundColor:
                  !referenceFile || !compareFile || isUiDisabled
                    ? "#374151"
                    : "#3B82F6",
                color:
                  !referenceFile || !compareFile || isUiDisabled
