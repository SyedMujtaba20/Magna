...
const handleAddFurnace = async () => {
  ...
  const fetchCampaigns = async (furnaceId) => {
    try {
      ...
      setScans([]);
      setIsLoading(true);
      const res = await axios.get(`${API_BASE}/api/campaigns?furnace_id=${furnaceId}`);
  ...
  const fetchScans = async (campaignId) => {
    try {
      ...
      setIsLoading(true);
      const res = await axios.get(`${API_BASE}/api/scans?campaign_id=${campaignId}`);
      setScans(res.data);
    } catch (err) {
...
{!selectedFurnace && (
  <>
    ...
    {furnaces.length > 0 ? (
      <ul>
        {furnaces.map(f => (
          <li key={f.furnace_id} onClick={() => fetchCampaigns(f.furnace_id)}>
    ...
    {showAddFurnace ? (
      <div style={{ marginTop: '1rem' }}>
        ...
        <button onClick={() => setShowAddFurnace(false)}>❌ Cancel</button>
      </div>
    ) : (
      <button onClick={() => setShowAddFurnace(true)}>➕ Add Furnace</button>
    )}
  </>
)}
        ...
        {selectedFurnace && !selectedCampaign && (
          <>
            ...
            <ul>
              {campaigns.map(c => (
                <li key={c.campaign_id} onClick={() => fetchScans(c.campaign_id)}>
                  {c.name || 'Unnamed Campaign'}
                </li>
              ))}
            </ul>
            <button onClick={() => alert('Add Campaign logic')}>➕ Add Campaign</button>
        ...
        {selectedCampaign && (
          <>
            ...
            <button onClick={() => setSelectedCampaign(null)}>🔙 Back to Campaigns</button>
          </>
        )}
