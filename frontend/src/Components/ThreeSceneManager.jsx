...
import { createSimpleControls, renderPointCloud } from "./threeUtils";
// 🆕 Add this import
import { updateThreeSceneForViewMode } from "./ThicknessVisualizationHelper";
...
const ThreeSceneManager = ({
  ...
  // 🆕 ADD THIS NEW useEffect for Mean/Minimum thickness visualization
  useEffect(() => {
    if (!sceneRef.current || !cameraRef.current || !selectedFile || activeScreen !== "3DView") {
      return;
    }
    ...
    console.log(`🎯 ThreeSceneManager: ViewMode changed to '${viewMode}'`);

    // Update 3D scene when viewMode changes to mean or minimum
    if (viewMode === "mean" || viewMode === "minimum") {
      console.log(`📊 Adding thickness labels for viewMode: ${viewMode}`);
      updateThreeSceneForViewMode(scene, camera, fileData, viewMode);
    } else {
      console.log(`🧹 Removing thickness labels (viewMode: ${viewMode})`);
      // Remove thickness labels when not in mean/minimum mode
  ...
  }, [viewMode, selectedFile, fileDataCache, activeScreen]);
