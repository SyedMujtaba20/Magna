...
const LidarVisualizer = () => {
  ...
  const [dataStats, setDataStats] = useState({ min: 0, max: 10, dataMin: 0, dataMax: 10 });
  const [globalDataRange, setGlobalDataRange] = useState({ min: Infinity, max: -Infinity, isInitialized: false });
  ...
  const [alarmState, setAlarmState] = useState({ robot: false, variator: false });
  ...
  const { mainCanvasRef, containerRef, zoomIn, zoomOut } = useControls(isUiDisabled);
  ...
  const sidebarProps = useMemo(() => ({
    ...
    handleStartCycle, handleStopCycle, handleResetCycle, handleSetTemplate,
    handleResetAlarms, handleScreenChange, isCycling, progress, isUiDisabled,
    templateData, alarmState, selectedFile, selectedFurnace
  ]);
  ...
  const threeSceneProps = useMemo(() => ({
    ...
    mainCanvasRef, containerRef, selectedFile, fileDataCache, useGlobalScaling,
    globalDataRange, wearRange, viewMode, showTemplate, showFurnace,
    selectedFurnace, activeScreen, templateData, isUiDisabled, selectedArea,
    zoomLevel, lensMode
  ]);
  ...
  const toolbarProps = useMemo(() => ({
    ...
    wearRange, viewMode, showTemplate, showFurnace, selectedArea,
    isUiDisabled, templateData, zoomLevel, lensMode
  ]);
  ...
  }, [activeScreen, fileDataCache, selectedFile, selectedFurnace, isUiDisabled, files, templateData]);
  ...
  , [files.length, activeScreen, files, selectedFile, fileDataCache, useGlobalScaling, globalDataRange, isUiDisabled]);
            ...
            <div style={styles.visualization}>
              <div ref={containerRef} style={styles.canvasContainer}>
                ...
                <div style={zoomPanelStyle}>
                  <button onClick={zoomIn} disabled={isUiDisabled} style={zoomButtonStyle}>
                    <ZoomIn size={16} />
                  </button>
                  <button onClick={zoomOut} disabled={isUiDisabled} style={zoomButtonStyle}>
                    <ZoomOut size={16} />
                  </button>
                </div>
              </div>
